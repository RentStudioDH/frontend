...
const ListCategories = () => {
  ...
  return (
    <div className="bg-white relative overflow-x-auto shadow-lg br-15">
      ...
      <table className="w-full paragraph text-left">
        <thead className="bg-accent txt-white paragraph">
          <tr>
            ...
            <th scope="col" className="text-white p-15">
              ID
            </th>
            <th scope="col" className="text-white p-15">
              Categoría
        ...
        <tbody>
          {currentCategories.map((category) => (
            <Cards
              ...
              data={{
                ...category,
                productCount: countProductsByCategory(category.id) || 0,
              }}
              openModal={openModal}
            />
          ))}
      ...
      <nav
        className="flex items-center flex-wrap justify-between p-15 g-15"
        aria-label="Table navigation"
      >
        <span className="txt-quaternary paragraph">
          Mostrando{" "}
          <span className="font-semibold txt-tertiary">
            {indexOfFirstCategory + 1}-
            {Math.min(indexOfLastCategory, totalCategories)}
          </span>{" "}
          de{" "}
          <span className="font-semibold txt-tertiary">{totalCategories}</span>
        </span>
        <ul className="flex items-center paragraph g-5">
          <li>
            <i
              className={`txt-tertiary hover:brightness-50 cursor-pointer fa-solid fa-angle-left ${
          ...
          {[...Array(totalPages).keys()].map((number) => (
            <li key={number + 1}>
              <span
                className={`hover:brightness-50 cursor-pointer p-2 ${
                  currentPage === number + 1
                    ? "font-semibold txt-tertiary"
                    : "txt-quaternary"
                }`}
          ...
          <li>
            <i
              className={`txt-tertiary hover:brightness-50 cursor-pointer fa-solid fa-angle-right ${
          ...
          <li>
            <select
              ...
              className="w-full p-1 border rounded bg-white txt-tertiary"
            >
