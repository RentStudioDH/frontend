  ...
  return (
    <div className="bg-white shadow-lg grid br-15 p-15 g-15">
      <h2 className="txt-accent bb-primary subtitle">
        <strong>Disponibilidad del producto:</strong>
      </h2>
      <div className="w-fit flex flex-col md:flex-row g-15">
        <ReactDatePicker selected={startDate} onChange={handleChange} startDate={startDate} endDate={endDate} selectsRange inline locale={es} monthsShown={2} className="w-full" filterDate={filterDate} highlightDates={highlightWithRanges} />
        <div className="flex flex-col g-15">
          <div className="grid g-10 fechas">
            <div>
              <h3 className="txt-tertiary subtitle">
            ...
            <p className="txt-tertiary paragraph">
              Costo total <span className="legal">(DíasxPrecio)</span>: <strong className="txt-primary">${total}</strong>
            </p>
            <p className="txt-quaternary paragraph">Stock disponible: {currentStock}</p>
            {currentStock <= 0 && (
              <p style={{ backgroundColor: "#56494E", color: "white" }} className="font-bold text-xl py-2 px-4">
                No hay unidades disponibles
              </p>
            )}
            {currentStock > 0 && (
              <button style={{ backgroundColor: "#A62639", color: "white" }} className="text-white py-2 px-4 rounded-md" onClick={rentarHandle}>
                Ir a Rentar
              </button>
            )}
          </div>
      ...
      {isModalVisible && <Modals type={modalType} visible={isModalVisible} onClose={closeModal} />}
    </div>
  );
};
